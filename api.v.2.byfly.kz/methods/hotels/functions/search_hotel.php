...
function searchBaseInfo($nameHotel, $country)
{
    ...
    if ($findHottel != null) {
        $hotelData = getHotelInfo($findHottel['hotelId']);

        if ($hotelData['type'] == 'succ') {
            return array(
                "type" => "succ",
...
function getHotelInfo($hotelId)
{
    ...
    if ($hotel->num_rows > 0) {
        ...
        if ($getDescDB->num_rows > 0) {
            $getDesc = $getDescDB->fetch_assoc();
            $hotelData['description-html'] = str_replace('"', '', base64_decode($getDesc['long_desc']));
            $hotelData['description-small'] = str_replace('"', '', base64_decode($getDesc['miny_desc']));
        }
        ...
        while ($getHotelRew = $getHotelRewDB->fetch_assoc()) {
            ...
            $getHotelRew['text_positive'] = str_replace('"', '', $getHotelRew['text_positive']);
            $getHotelRew['text_negative'] = str_replace('"', '', $getHotelRew['text_negative']);
            $getHotelRew['title'] = str_replace('"', '', $getHotelRew['title']);
