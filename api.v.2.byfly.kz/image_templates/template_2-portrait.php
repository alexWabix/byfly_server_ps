...
if (empty($_GET['citys']) == false) {
    ...
    foreach ($arrayCitys as $cityId) {
        ...
        if ($datae['hottours']['hotcount'] > 0) {
            ...
            $dates[] = 'Вылет из ' . $tour['departurenamefrom'] . ' - ' . $tour['flydate'] . ', ночей - ' . $tour['nights'] . ', <br><strong>Стоимость - от ' . number_format($tour['price'], 0, '', ' ') . ' <small>₸</small></strong> / 1 чел. (' . $tour['meal'] . ') <div style="
    width: 100%;
    height: 4px;
    margin-bottom: 30px;
    margin-top: 30px;
